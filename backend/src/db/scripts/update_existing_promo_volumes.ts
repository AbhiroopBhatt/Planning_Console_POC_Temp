...
async function updateExistingPromoVolumes(promoIds?: number[]) {
  const client = await pcDbPool.connect();
  try {
    console.log('=== Updating Existing Promo Volumes ===');
    
    // Get active model
    const activeModel = await mlService.getActiveModel();
    if (!activeModel) {
      console.error('❌ No active ML model found. Cannot update volumes.');
      return;
    }
    ...
    let query = `
      ...
      WHERE fp.base_volume_snapshot IS NOT NULL
        AND fp.promo_volume_estimate IS NULL
    `;
    
    if (promoIds && promoIds.length > 0) {
      // Find promotion groups for the specified promo_ids
      const promoGroups = await client.query(`
        ...
        target_groups AS (
          SELECT DISTINCT r.discount_pct, r.grp
          FROM ranked r
          INNER JOIN target_promos tp ON r.promo_id = tp.promo_id
        ),
        group_ranges AS (
          SELECT MIN(r.promo_id) as min_promo_id, MAX(r.promo_id) as max_promo_id
          FROM ranked r
          INNER JOIN target_groups tg ON r.discount_pct = tg.discount_pct AND r.grp = tg.grp
        )
        SELECT min_promo_id, max_promo_id FROM group_ranges
      `, [promoIds]);
      
      if (promoGroups.rows.length > 0) {
        const ranges = promoGroups.rows.map(r => `(fp.promo_id >= ${r.min_promo_id} AND fp.promo_id <= ${r.max_promo_id})`).join(' OR ');
        query += ` AND (${ranges})`;
      }
    }
    
    query += ' ORDER BY fp.promo_id';
...
if (require.main === module) {
  const promoIds = process.argv.slice(2).map(id => parseInt(id)).filter(id => !isNaN(id));
  
  updateExistingPromoVolumes(promoIds.length > 0 ? promoIds : undefined)
    .then(() => {
      console.log('\n✓ Script complete');
      process.exit(0);
    })
...
export { updateExistingPromoVolumes };
