...
export default function PricesReportPage() {
  ...
  const { data: pricesData, isLoading: pricesLoading, refetch: refetchPrices } = useQuery({
    queryKey: ['prices-report', selectedCustomerIds, selectedProductIds, selectedTimeIds],
    queryFn: () => {
      const params = new URLSearchParams()
      ...
      return api.get(`/api/fact-data/prices?${params.toString()}`)
    },
  ...
  const hasActiveFilters = selectedCustomerIds.length > 0 || selectedCategoryIds.length > 0 || 
                          selectedBrandIds.length > 0 || selectedProductIds.length > 0
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-gray-900 dark:text-gray-100">Prices Report</span>
        </nav>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Analyze pricing data across customer and product hierarchies
            </p>
          </div>
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                      {prices.length > 0
                        ? `$${(
                            prices.reduce((sum, p) => sum + (Number(p.customer_price) || Number(p.list_price) || 0), 0) /
                            prices.length
                          ).toFixed(2)}`
                        : '$0.00'}
                    </dd>
                  ...
                  {prices.map((price, idx) => (
                    <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                        {price.list_price !== null && price.list_price !== undefined ? `$${Number(price.list_price).toFixed(2)}` : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-gray-900 dark:text-gray-100">
                        {price.customer_price !== null && price.customer_price !== undefined ? `$${Number(price.customer_price).toFixed(2)}` : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                        {price.base_price !== null && price.base_price !== undefined ? `$${Number(price.base_price).toFixed(2)}` : '-'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                        {price.promo_price !== null && price.promo_price !== undefined ? `$${Number(price.promo_price).toFixed(2)}` : '-'}
                      </td>
                    </tr>
                  ))}
                </tbody>
