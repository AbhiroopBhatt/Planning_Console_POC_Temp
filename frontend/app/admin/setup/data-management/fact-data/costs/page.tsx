...
export default function CostsReportPage() {
  ...
  const { data: costsData, isLoading: costsLoading, refetch: refetchCosts } = useQuery({
    queryKey: ['costs-report', selectedCustomerIds, selectedProductIds, selectedTimeIds],
    queryFn: () => {
      const params = new URLSearchParams()
      ...
      return api.get(`/api/fact-data/costs?${params.toString()}`)
    },
    enabled: selectedCustomerIds.length > 0 || selectedProductIds.length > 0 || selectedTimeIds.length > 0,
  })
  ...
  const timeHierarchy: TimeYear[] = timeData?.data?.data?.hierarchy || []
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-gray-900 dark:text-gray-100">Costs Report</span>
        </nav>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Analyze cost data across customer and product hierarchies
            </p>
          </div>
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                      {costs.length > 0
                        ? `$${(
                            costs.reduce((sum, c) => sum + (Number(c.cogs) || 0), 0) /
                            costs.length
                          ).toFixed(2)}`
                        : '$0.00'}
                    </dd>
            ...
            {costsLoading ? (
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {costs.map((cost, idx) => {
                    const totalCost = (Number(cost.cogs) || 0) + (Number(cost.logs) || 0)
                    return (
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                          {cost.cogs !== null && cost.cogs !== undefined ? `$${Number(cost.cogs).toFixed(2)}` : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                          {cost.logs !== null && cost.logs !== undefined ? `$${Number(cost.logs).toFixed(2)}` : '-'}
                        </td>
