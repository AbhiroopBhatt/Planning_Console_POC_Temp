...
export default function CostsReportPage() {
  ...
  // Fetch costs report from DuckDB
  const { data: costsData, isLoading: costsLoading, refetch: refetchCosts } = useQuery({
    queryKey: ['duckdb-costs-report', selectedCustomerIds, selectedCategoryIds, selectedBrandIds, selectedProductIds, selectedTimeIds],
    queryFn: () => {
      const params = new URLSearchParams()
      ...
      return api.get(`/api/duckdb/reports/costs?${params.toString()}`)
    },
  ...
  const costs: CostRecord[] = costsData?.data?.data?.costs || []
  const reportStats = costsData?.data?.data || {}
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-gray-900 dark:text-gray-100">Costs Report (DuckDB)</span>
        </nav>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Analyze cost data from DuckDB Costs_allcombo_view with customer rollup and drilldown
            </p>
          </div>
        ...
        {/* Global Filter Bar - Same as prices report */}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                      {reportStats.recordsWithData > 0 && reportStats.totals?.cogs
                        ? `$${(
                            reportStats.totals.cogs / reportStats.recordsWithData
                          ).toFixed(2)}`
                        : '$0.00'}
                    </dd>
                  </dl>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <div className="overflow-x-auto">
            {costsLoading ? (
              <div className="px-6 py-12 text-center text-gray-500">Loading costs...</div>
            ) : costs.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <BarChart3 className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-500 dark:text-gray-400">
                  {hasActiveFilters
                    ? 'No cost data found for selected filters'
                    : 'Select customers and/or products to view costs'}
                </p>
              </div>
            ) : (
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {costs.map((cost, idx) => {
                    const totalCost = (cost.cogs || 0) + (cost.logs || 0)
                    return (
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            {cost.has_data === 1 ? 'Yes' : 'No'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                          {cost.cogs !== null ? `$${cost.cogs.toFixed(2)}` : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                          {cost.logs !== null ? `$${cost.logs.toFixed(2)}` : '-'}
                        </td>
