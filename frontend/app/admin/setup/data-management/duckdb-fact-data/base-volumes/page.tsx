...
import { ChevronRight, Filter, Download, RefreshCw, ChevronDown, ChevronUp, X, Search, BarChart3, Users, Package, TrendingUp } from 'lucide-react'
...
export default function BaseVolumesReportPage() {
  ...
  // Fetch base volumes report from DuckDB
  const { data: volumesData, isLoading: volumesLoading, refetch: refetchVolumes } = useQuery({
    queryKey: ['duckdb-base-volumes-report', selectedCustomerIds, selectedCategoryIds, selectedBrandIds, selectedProductIds, selectedTimeIds],
    queryFn: () => {
      const params = new URLSearchParams()
      ...
      return api.get(`/api/duckdb/reports/base-volumes?${params.toString()}`)
    },
  ...
  const volumes: VolumeRecord[] = volumesData?.data?.data?.volumes || []
  const reportStats = volumesData?.data?.data || {}
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <nav className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400">
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-gray-900 dark:text-gray-100">Base Volumes Report (DuckDB)</span>
        </nav>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Analyze base volume data from DuckDB Basevolume_allcombo_view with customer rollup and drilldown
            </p>
          </div>
        ...
        {/* Global Filter Bar - Same as prices/costs report */}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                      {reportStats.totals?.volume
                        ? Number(reportStats.totals.volume).toLocaleString(undefined, { maximumFractionDigits: 2 })
                        : '0'}
                    </dd>
                  </dl>
                </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
          ...
          <div className="overflow-x-auto">
            {volumesLoading ? (
              <div className="px-6 py-12 text-center text-gray-500">Loading volumes...</div>
            ) : volumes.length === 0 ? (
              <div className="px-6 py-12 text-center">
                <BarChart3 className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-500 dark:text-gray-400">
                  {hasActiveFilters
                    ? 'No volume data found for selected filters'
                    : 'Select customers and/or products to view volumes'}
                </p>
              </div>
            ) : (
                    ...
                    return (
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-xs text-gray-500 dark:text-gray-400">
                            ID: {volume.product_id}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                            ID: {volume.customer_id}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-gray-900 dark:text-gray-100">
                            ID: {volume.rollup_customer_id}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                          {volume.date ? new Date(volume.date).toLocaleDateString() : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            {volume.has_data === 1 ? 'Yes' : 'No'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-900 dark:text-gray-100">
                          {volume.volume !== null ? Number(volume.volume).toLocaleString(undefined, { maximumFractionDigits: 2 }) : '-'}
                        </td>
                      </tr>
                    )
                  })}
